...
void Node_Split(node* s, int i, int t){			//Разделение узла на два
...
void add(Btree* Tr, int k){						//Добавление ключа в дерево
	node* r = Tr->root;
	if(!keyFinder(r, k)){
		if (r->n == 2 * Tr->t - 1){
...
void node_turn(node* r, int c, _Bool direction){
	node* p = r->parent;
	if(direction == tRIGHT){		//Перенос(поворот) ключа справа
		...
		r->n += 1;
		nbhd->n -= 1;
	}else if(direction == tLEFT){	//Перенос(поворот) ключа слева
		node* nbhd = p->childs[c-1];
		for(int j = r->n; j >= 1; j--){
			r->keys[j] = r->keys[j-1];
		}
...
void treeCorrect(node* r, int t){ //Корректировка дерева, если в узле меньше t-1 ключей
	if(r->n == 0){			//Если 0 ключей, то корнем становится единственный ребёнок
		r = r->childs[0];
	}else if(r->parent == NULL){ //Если это корень, то всё хорошо
		return;
	}else{
		node* p = r->parent;
		int c_ind = 0;
		...
		if((right) && (p->childs[c_ind+1]->n > t-1)){     //Поворочиваем, либо делаем слияние
...
void delete(node* rt, int k, int t){			//Удаление ключа
	node* r = keyFinder(rt, k);		//Находим нужный узел
	if(!r){
		return;
	}
	int key_num = r->n;			//находим индекс нужного элемента
	int key_ind = 0;
	while(r->keys[key_ind] != k){
		key_ind++;
	}

	if(r->leaf){				//удаляем из листа, если стало меньше t-1 элементов, то делаем корректировку
		...
		node* child = r->childs[key_ind+1];	//Ищем самый левый лист у соседа справа
		while(!child->leaf){
...
void tree_print(node* r){					//Вывод дерева
	if(!r->leaf){
		for(int i = 0; i <= r->n; i++){
